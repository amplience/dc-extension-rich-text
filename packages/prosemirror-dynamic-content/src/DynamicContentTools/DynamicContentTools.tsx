...
export function dcContentLink(
  ...
  return {
    ...
    apply: async (
      ...
      try {
        const value = await richTextEditorContext.dialogs.getDcContentLink(
          contentTypes
        );
        dispatch(
          state.tr.replaceSelectionWith(
...
export function createDynamicContentTools(
  schema: any,
  ...
  if (
    isToolEnabled("dc-image-link", options) &&
  ...
  if (
    isToolEnabled("dc-content-link", options) &&
